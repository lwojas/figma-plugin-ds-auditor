...
type Config = {
  designSystemComponentKeys: StringMap;
  dsysLibraryOnlyComponentKeys: StringMap;
  designSystemTextStyleKeys: StringMap;
  designSystemColorVariableKeys: NumberMap;
  libraryComponentSets: StringMap;
  deprecatedColors: StringMap;
  deprecatedTypography: StringMap;
  figmaApiKey: string;
  ignoreCache: boolean;
  designSystemFiles: {
    components: string[];
...
type AuditResults = {
  ...
  dsysComponents: string[];
  nonDsysComponents: { name: string; id: string }[];
  uniqueDsysComponents: NumberMap;
  uniqueNonDsysComponents: NumberMap;
  framesWithoutAutoLayout: string[];
  nonDsysColorFills: { name: string; id: string }[];
  totalColors: number;
  nonDsysColorTokens: NumberMap;
  dsysColorTokens: NumberMap;
  totalDsysColorTokens: number;
  totalTextNodes: number;
  dsysTextStyles: StringMap;
  nonDsysTextStyles: Record<string, { name: string; id: string }>;
  nonDsysText: { name: string; id: string }[];
  dsysStyledTextCount: number;
  nonDsysStyledTextCount: number;
  unstyledTextCount: number;
};
...
function sanitiseComponentKey(key: string) {
  if (typeof key !== "string") return "";
  let sanitiseKey = key.replace(/^.*?:/, "");
  sanitiseKey = sanitiseKey.replace(/\/.*/, "");
  return sanitiseKey;
...
async function loadDesignSystemData() {
  async function fetchDeprecatedStyles(fileKey: string) {
  ...
  async function fetchFileStyles(fileKey: string, type: string) {
  ...
  async function fetchComponents(filekeys: string[]) {
    for (let i = 0; i < filekeys.length; i++) {
      ...
      const components: { key: string; name: string }[] = Object.values(
        data.components,
      );
  ...
  async function fetchLibraryComponents(filekeys: string[]) {
    for (let i = 0; i < filekeys.length; i++) {
      try {
        const response = await fetch(
        ...
        const data = await response.json();
        // console.log(data);
        const components: any = Object.values(data.meta.components);
  ...
  async function fetchFileColorVariables(fileKey: string) {
    const response = await fetch(
